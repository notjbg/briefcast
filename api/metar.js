...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!upstream.ok) {
      console.error('briefcast.metar.upstream_error', { status: upstream.status });
      return json(res, 502, { error: 'Upstream METAR service unavailable' }, 10);
    }
    const data = await upstream.json();
    return json(res, 200, { ids: codes, metars: Array.isArray(data) ? data : [] }, 120);
  } catch (error) {
    console.error('briefcast.metar.error', error.message);
    return json(res, 500, { error: 'Failed to fetch METAR data' }, 10);
  }
};
